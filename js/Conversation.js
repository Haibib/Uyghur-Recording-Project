...
function empDetails(xml) { 
    ...
    for(j = 1; j < metadata.childNodes.length-4; j+=2){
        metadataString += '<div class = metadataElement '+metadata.childNodes[j].tagName+'"><span class="metadataTitle">'+metadata.childNodes[j].tagName.substring(0, 1).toUpperCase()+metadata.childNodes[j].tagName.substring(1)+'</span>' + metadata.childNodes[j].innerHTML + '</div>';
    }
    metadataString += '<div class = "metadataSubtitle '+metadata.childNodes[j].tagName+'"> '+metadata.childNodes[j].tagName.substring(0, 1).toUpperCase()+metadata.childNodes[j].tagName.substring(1)+'</div>';
    let metadataSubString = metadata.childNodes[j].childNodes[1];
    metadataString += '<div class = "metadataElement '+metadataSubString.tagName+'"><span class="metadataTitle">'+metadataSubString.tagName.substring(0, 1).toUpperCase()+metadataSubString.tagName.substring(1)+'</span>' + metadataSubString.innerHTML + '</div>';
    metadataSubString = metadata.childNodes[j].childNodes[3];
    metadataString += '<div class = "metadataElement '+metadataSubString.tagName+'"><span class="metadataTitle">'+metadataSubString.tagName.substring(0, 1).toUpperCase()+metadataSubString.tagName.substring(1)+'</span>' + metadataSubString.innerHTML + '</div>';
    j+=2;
    metadataString += '<div class = "metadataSubtitle '+metadata.childNodes[j].tagName+'"> '+metadata.childNodes[j].tagName.substring(0, 1).toUpperCase()+metadata.childNodes[j].tagName.substring(1)+'</div>';
    metadataSubString = metadata.childNodes[j].childNodes[1];
    metadataString += '<div class = "'+metadataSubString.tagName+' metadataElement"><span class="metadataTitle"><span class="metadataTop">'+metadataSubString.tagName.substring(0, 1).toUpperCase()+metadataSubString.tagName.substring(1,7)+'</span><br><span class="metadataBottom">'+metadataSubString.tagName.substring(8, 9).toUpperCase()+metadataSubString.tagName.substring(9)+'</span></span>' + metadataSubString.innerHTML + '</div>';
    metadataSubString = metadata.childNodes[j].childNodes[3];
    metadataString += '<div class = "metadataElement '+metadataSubString.tagName+'"><span class="metadataTitle">'+metadataSubString.tagName.substring(0, 1).toUpperCase()+metadataSubString.tagName.substring(1)+'</span>' + metadataSubString.innerHTML + '</div>';
    metadataSubString = metadata.childNodes[j].childNodes[5];
    metadataString += '<div class = "metadataElement '+metadataSubString.tagName+'"><span class="metadataTitle">'+metadataSubString.tagName.substring(0, 1).toUpperCase()+metadataSubString.tagName.substring(1)+'</span>' + metadataSubString.innerHTML + '</div>';
    metadataSubString = metadata.childNodes[j].childNodes[7];
    metadataString += '<div class = "'+metadataSubString.tagName+' metadataElement"><span class="metadataTitle"><span class="metadataTop">'+metadataSubString.tagName.substring(0, 1).toUpperCase()+metadataSubString.tagName.substring(1,3)+'</span><br><span class="metadataBottom">'+metadataSubString.tagName.substring(4, 5).toUpperCase()+metadataSubString.tagName.substring(5)+'</span></span>' + metadataSubString.innerHTML + '</div>';
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let conversationBody = '<div id="main-content" class="container"> <div id="container" class="container"> <div id="homepage-container" class="row"> <h1 class="title text-center"> Conversation </h1> <div id="toggle-collection" class=""> <div class="hold form-check form-switch"> <div id="check-holder"> <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckChecked" checked> </div> <label class="form-check-label" for="flexSwitchCheckDefault">Intonational Unit </label> </div> <div class="hold form-check form-switch"> <div id="check-holder"> <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckChecked" checked> </div> <label class="form-check-label" for="flexSwitchCheckDefault">Gloss </label> </div> <div class="hold form-check form-switch"> <div id="check-holder"> <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckChecked" checked> </div> <label class="form-check-label" for="flexSwitchCheckDefault">Segment </label> </div> <div class="hold form-check form-switch"> <div id="check-holder"> <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckChecked" checked> </div> <label class="form-check-label" for="flexSwitchCheckDefault">English Translation </label> </div> </div> <div id="conversation-page-container" class="container"> <div id="audio-container"> <audio class="recording" src="" controls="true"></audio> </div> </div> <a class="scroll" href="#metadata"> Scroll to bottom for metadata </a> <div id = "table"> </div> <a class="scroll" href="#audio-container"> Scroll to top </a> <div id="metadata"> <div class="title text-center"> MetaData </div> </div> </div> </div> </div>';
    let html = document.querySelector("html");
    html.innerHTML = conversationHeader + conversationBody + html.innerHTML;
    this.getElementsByClassName("recording")[0].src = "audio/"+conversation.replace(/\s/g, '')+"_redacted.mp3";
    document.getElementsByClassName("title")[0].innerText= conversation;
    let conversationFooter = '<div class="container"> <div class="row"> <section id="volunteer" class="col-md-4"> <span>Volunteer: </span> <br> Sign consent form <br> Record conversation <br> Contact Michael Fiddler <hr class="visible-xs text-center"> </section> <section id="contact" class="col-md-4"> <span>Contact: </span> <br> Email - <a href="mfiddler@ucsb.edu">mfiddler@ucsb.edu</a> <br> Web design - Haibibullah Abdul-Kerim <p>*Project organized by Michael Fiddler, in collaboration with the <a href="https://uyghursfoundation.org/en/">Uyghur Projects Foundation</a></p> <hr class="visible-xs text-center"> </section> <section id="random" class="col-md-4"> <p>"Great research material to linguists worldwide"</p> <p>"Helping to perserve the Uyghur language"</p> </section> </div> <div class="text-center">© Copyright Michael Fiddler 2022 </div> </div>';
